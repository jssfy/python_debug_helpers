...
以 **test (3.13)** 为例，定义了 **7 个步骤**，实际执行 **12 个步骤**：
...
| **调试步骤** | 无 | 2 个（显示 Python 版本、显示已安装包） |
...
```
         lint (22s)              test 3.13 (19s)
安装依赖  11s ████████████       7s ████████         ← test 不装 lint 工具，更快
核心步骤   6s ██████             0s                   ← 测试用例少（3 个）
调试步骤   0s                    3s ███               ← lint 无调试步骤
其他       5s █████              9s █████████         ← test 多了升级 pip + Codecov 判断
```

test 安装依赖更快（7s vs 11s），因为 pytest 运行时不需要 ruff/pyright/mypy。但实际上当前 `[dev]` 依赖组包含了所有工具，test 也安装了它们。如果想优化，可以将 lint 工具拆到单独的依赖组（如 `[lint]`），让 test job 只装 `[dev]`。
